...
static void calculateColorMapFromChange(const scp chunk, bval *change, colormap *map){
...
//Used to display the playing field.
...
static bval getv(const scp chunk, int x, int y){
	if(!chunk) return 0;
	int dx = 0, dy = 0;
